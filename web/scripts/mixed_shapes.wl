...
model = CreatePhysicsModel[{
  ...
  (* Dynamic cuboid, slightly tilted *)
  DynamicBody[{MaterialShading["Copper"],
    Cuboid[{-2, -0.5, 4}, {-1, 0.5, 5}]},
    "Orientation" -> RotationMatrix[Pi/6, {0, 1, 1}]],

  (* Dynamic sphere *)
  DynamicBody[{MaterialShading["Gold"], Sphere[{0, 0, 5.5}, 0.5]},
    "Density" -> 2.0],

  (* Dynamic cylinder *)
  DynamicBody[{MaterialShading["Silver"],
    Cylinder[{{1.5, 0, 4.5}, {1.5, 0, 5.5}}, 0.35]}],

  (* Dynamic cone *)
  DynamicBody[{MaterialShading["Brass"],
    Cone[{{-1, 1, 5}, {-1, 1, 6}}, 0.4]}]
},
  "Gravity" -> {0, 0, -9.81},
...
outFile = FileNameJoin[{outDir, "mixed_shapes.mp4"}];
